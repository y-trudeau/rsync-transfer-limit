...
#endif
static char *bwlimit_arg, *max_size_arg, *min_size_arg, *transfer_limit_arg;
...
void usage(enum logcode F)
{
  ...
  rprintf(F,"     --transfer-limit=NBYTES limit bandwidth transfer size\n");
#ifdef HAVE_SETVBUF
...
enum {OPT_VERSION = 1000, OPT_DAEMON, OPT_SENDER, OPT_EXCLUDE, OPT_EXCLUDE_FROM,
      ...
      OPT_USERMAP, OPT_GROUPMAP, OPT_CHOWN, OPT_BWLIMIT, OPT_TRANSFER_LIMIT, 
...
static struct poptOption long_options[] = {
  ...
  {"transfer-limit",   0,  POPT_ARG_STRING, &transfer_limit_arg, OPT_TRANSFER_LIMIT, 0, 0 },
...
int parse_arguments(int *argc_p, const char ***argv_p)
{
	...
	while ((opt = poptGetNextOpt(pc)) != -1) {
		...
		case OPT_TRANSFER_LIMIT:
			{
				OFF_T limit = parse_size_arg(&transfer_limit_arg, 'B');
				if (limit < 0) {
					snprintf(err_buf, sizeof err_buf,
						"--transfer-limit value is invalid: %s\n", bwlimit_arg);
					return 0;
				}
